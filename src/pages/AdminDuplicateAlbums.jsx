            ...
            <div key={key} style={{ backgroundColor: 'var(--color-bg-surface)', borderRadius: '0.5rem', padding: '1rem', marginBottom: '1rem', boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.1)' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-start' }}>
                <div style={{ flex: '1 1 200px' }}>
                  ...
                  <p style={{ fontSize: '0.875rem', color: 'var(--color-text-muted)' }}>{pair.a.artist_name}{pair.a.release_year ? ` · ${pair.a.release_year}` : ''} · {pair.a.track_count} track{pair.a.track_count === 1 ? '' : 's'}</p>
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    ...
                    <button
                      disabled={busy}
                      onClick={() => handleDelete(pair, pair.a)}
                      style={{ marginTop: '0.5rem', padding: '0.4rem 0.75rem', backgroundColor: '#ef4444', color: 'white', border: 'none', borderRadius: '4px', cursor: busy ? 'not-allowed' : 'pointer', fontSize: '0.8rem' }}
                    >
                      Delete
                    </button>
                  </div>
                ...
                <div style={{ flex: '1 1 200px' }}>
                  ...
                  <p style={{ fontSize: '0.875rem', color: 'var(--color-text-muted)' }}>{pair.b.artist_name}{pair.b.release_year ? ` · ${pair.b.release_year}` : ''} · {pair.b.track_count} track{pair.b.track_count === 1 ? '' : 's'}</p>
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <button
                      disabled={busy}
                    ...
                    <button
                      disabled={busy}
                      onClick={() => handleDelete(pair, pair.b)}
                      style={{ marginTop: '0.5rem', padding: '0.4rem 0.75rem', backgroundColor: '#ef4444', color: 'white', border: 'none', borderRadius: '4px', cursor: busy ? 'not-allowed' : 'pointer', fontSize: '0.8rem' }}
                    >
                      Delete
                    </button>
