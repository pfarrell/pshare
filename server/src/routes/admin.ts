...
admin.post('/album/:id/images', async (c) => {
  ...
  try {
    const filePath = await downloadToDisk(image_url, image_name, 'albums')
...
admin.post('/artist/:id/images', async (c) => {
  ...
  try {
    const filePath = await downloadToDisk(image_url, image_name, 'artists')
    const image = await createImageRecord('artist', id, filePath, 'manual', set_primary)
    return c.json({ success: true, image })
  } catch (err) {
    if (err instanceof ImageStorageError) return c.json({ error: err.message }, 400)
    console.error('Error adding artist image:', err)
    return c.json({ error: 'Failed to save image' }, 500)
  }
})
